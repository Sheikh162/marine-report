// app/admin/[reportId]/edit/_components/EditReportForm.tsx
'use client';
...
export function EditReportForm({ report }: EditReportFormProps) {
  ...
  const methods = useForm<ReportInput>({
    ...
    defaultValues: {
      ...
      yearBuilt: report.yearBuilt ? new Date(report.yearBuilt) : undefined,
  ...
  const onSubmit = async (data: ReportInput) => {
    setIsLoading(true);
    try {
/*       await axios.put(`/api/admin/reports/${report.id}`, data);
      toast({
        title: "Success!",
